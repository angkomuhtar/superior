        ...
        <div class="card card-custom gutter-b">
            <div class="card-header flex-wrap py-3">
                ...
                <div class="card-toolbar">
                    ...
                    <!--end::Dropdown-->
                    <!--begin::Button-->
                    <a href="{{route('dashboard.tes.add')}}" class="btn btn-primary font-weight-bolder">
            ...
            <div class="card-body">
                ...
                <table class="table table-bordered table-checkable" id="kt_datatable">
                    ...
                    <tr>
                        <th>Nama Tes</th>
                        <th>Secret Key</th>
                        <th>Waktu Tiap Kolom</th>
                        <th>Nama Kelompok Soal</th>
                        <th>Total Waktu Tes</th>
                        <th>Status</th>
                        <th></th>
                    </tr>
                    </thead>
                    ...
                    @foreach ($tes AS $item)
                        <tr>
                            ...
                            <td>
                                @if ($item->kelompokSoal)
                                {{$item->kelompokSoal->nama_kelompok_soal}} ({{$item->kelompokSoal->kolom->count()}} Kolom)
                                @endif
                            </td>
                            <td>
                                @if ($item->kelompokSoal)
                                {{$item->batas_waktu * $item->kelompokSoal->kolom->count()}} Menit
                                @endif
                            </td>
                            <td>{{$item->aktif ? 'Aktif' : 'Tidak Aktif'}}</td>
                            <td>
                                <a href="{{route('dashboard.tes.edit',$item)}}" class="btn btn-info"><span class="fa fa-edit"></span> Edit</a>
                                <a href="javascript:void(0)" class="btn btn-{{$item->aktif ? 'warning' : 'primary'}} btn-toggle-aktif" data-id="{{$item->id}}"><span class="fa fa-{{$item->aktif ? 'times' : 'check'}}"></span> {{$item->aktif ? 'Nonaktifkan' : 'Aktifkan'}}</a>
